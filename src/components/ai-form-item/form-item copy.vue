  ...
  <template v-for="(r, index) in columns">
    ...
    <a-form-item v-if="r[mode]" :label="r.title" :field="r.dataIndex" :class="[mode, r?.key]">
      ...
      <a-input-number v-else-if="r.view_type === 'number'" v-model="formData[r.dataIndex]"
        :placeholder="`请输入${r.title}`" allow-clear
        @input="change($event, r.dataIndex)" />

      <!-- 浮点数框 -->
      <a-input-number v-else-if="r.view_type === 'float'" v-model="formData[r.dataIndex]"
        :precision="2" :placeholder="`请输入${r.title}`" allow-clear
        @input="change($event, r.dataIndex)" />
      ...
      <!-- 滑块 -->
      <a-slider v-else-if="r.view_type === 'slider'" v-model="formData[r.dataIndex]" @input="change($event, r.dataIndex)"  />

      <!-- 文本域 -->
      <a-textarea v-else-if="r.view_type === 'textarea'" v-model="formData[r.dataIndex]" :placeholder="`请输入${r.title}`"
        allow-clear @input="change($event, r.dataIndex)" />
      ...
      <!-- 上传图片 -->
      <ai-upload-image v-else-if="r.view_type === 'upimage'" v-model="formData[r.dataIndex]" :mode="r?.view_mode"  @input="change($event, r.dataIndex)" />

      <!-- 上传文件 -->
      <ai-upload-file v-else-if="r.view_type === 'upfile'" v-model="formData[r.dataIndex]" :mode="r?.view_mode"  @input="change($event, r.dataIndex)" />

      <!-- 字典组合框 -->
      <ai-dict v-else-if="r.view_type === 'dict'" v-model="formData[r.dataIndex]" :data-type="r.data_type"  @input="change($event, r.dataIndex)" />

      <!-- 图标选择器 -->
      <ai-icon v-else-if="r.view_type === 'icon'" v-model="formData[r.dataIndex]"  @input="change($event, r.dataIndex)" />
